...
int itree_init(struct inode_tree *itree, int num_items)
{
	itree->sorted = RB_ROOT;
	itree->inodes = RB_ROOT;

	if (num_items <= 0 || num_items > 65535) {
		fprintf(stderr, "itree: invalid fetch granularity\n");
		return -1;
	}

	itree->num_items = num_items;
	itree->buf = malloc(num_items * sizeof(struct duet_item));
	if (!(itree->buf)) {
		fprintf(stderr, "itree: error allocating item buffer\n");
		return -1;
	}
