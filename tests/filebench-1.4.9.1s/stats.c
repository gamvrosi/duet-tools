...
static struct flowstats *oldstats = NULL;
...
stats_snap(void)
{
	...
	/* Added */
	struct flowstats *oldiostat = &oldstats[FLOW_TYPE_IO];
	struct flowstats *oldaiostat = &oldstats[FLOW_TYPE_AIO];
	struct flowstats *oldglstat = &oldstats[FLOW_TYPE_GLOBAL];
	...
	*str = '\0';
	...
	filebench_log(LOG_INFO,
	    ...
	    iostat->fs_count + aiostat->fs_count, /* ops */
	    (iostat->fs_count + aiostat->fs_count) /
	    ((globalstats->fs_etime - globalstats->fs_stime) / FSECS), /* ops/s */
	    (iostat->fs_rcount + aiostat->fs_rcount) /
	    ((globalstats->fs_etime - globalstats->fs_stime) / FSECS), /* R/w */
	    (iostat->fs_wcount + aiostat->fs_wcount) /
	    ((globalstats->fs_etime - globalstats->fs_stime) / FSECS), /* r/W */
	    //((iostat->fs_bytes + aiostat->fs_bytes) / (1024 * 1024)) /
	    //((globalstats->fs_etime - globalstats->fs_stime) / FSECS), /* mb/s */
	    (((iostat->fs_bytes + aiostat->fs_bytes) - (oldiostat->fs_bytes + oldaiostat->fs_bytes)) / (1024 * 1024)) / // ADDED
	    (((globalstats->fs_etime - globalstats->fs_stime) - (oldstats->fs_etime - oldstats->fs_stime)) / FSECS), /* mb/s */
	    (iostat->fs_rcount + iostat->fs_wcount +
	    aiostat->fs_rcount + aiostat->fs_wcount) ?
	    (iostat->fs_syscpu / 1000.0) /
	    (iostat->fs_rcount + iostat->fs_wcount +
	    aiostat->fs_rcount + aiostat->fs_wcount) : 0, /* us cpu/op */
	    //(iostat->fs_rcount + iostat->fs_wcount) ?
	    //iostat->fs_mstate[FLOW_MSTATE_LAT] /
	    //((iostat->fs_rcount + iostat->fs_wcount) * 1000000.0) : 0);
	    ((iostat->fs_rcount + iostat->fs_wcount) - (oldiostat->fs_rcount + oldiostat->fs_wcount)) ? // ADDED
	    (iostat->fs_mstate[FLOW_MSTATE_LAT] - oldiostat->fs_mstate[FLOW_MSTATE_LAT]) /
	    (((iostat->fs_rcount + iostat->fs_wcount) - (oldiostat->fs_rcount + oldiostat->fs_wcount)) * 1000000.0) : 0); /* us cpu/op */
...
stats_clear(void)
{
	...
	if (globalstats == NULL) {
		...
		/* Added */
		memset(globalstats, 0, FLOW_TYPES * sizeof (struct flowstats));
	...
	if (oldstats == NULL)
		oldstats = malloc(FLOW_TYPES * sizeof (struct flowstats));

	memcpy(oldstats, globalstats, FLOW_TYPES * sizeof (struct flowstats)); //ADDED
	oldstats->fs_etime = gethrtime(); //ADDED
	//(void) memset(globalstats, 0, FLOW_TYPES * sizeof (struct flowstats));
